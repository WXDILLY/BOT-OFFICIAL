// main.js - Banca Toscana + Sistema Ticket Prato RP
...
const RUOLO_RICHIESTA_ID = process.env.RUOLO_RICHIESTA_ID || "1523264734697099385";
const RUOLO_APPROVATO_ID = process.env.RUOLO_APPROVATO_ID || "1530306848863968469";
...
client.once('ready', async () => {
  ...
  try {
    ...
    await pool.query(`ALTER TABLE banca_toscana ADD COLUMN IF NOT EXISTS status VARCHAR(50) DEFAULT 'active';`);
    await pool.query(`ALTER TABLE banca_toscana ADD COLUMN IF NOT EXISTS nome_cognome VARCHAR(255);`);
    await pool.query(`ALTER TABLE banca_toscana ADD COLUMN IF NOT EXISTS nome_roblox VARCHAR(255);`);
    ...
    await pool.query(
      "INSERT INTO banca_toscana (user_id, saldo, status) VALUES ('TESORERIA_PRATO', $1, 'active') ON CONFLICT (user_id) DO NOTHING;",
      [capitaleTesoreria]
    );
...
async function gestisciTassazioneSettimanale() {
  ...
  try {
    ...
    const utenti = await clientDb.query(
      "SELECT user_id, saldo FROM banca_toscana WHERE user_id != 'TESORERIA_PRATO' AND saldo > 0 AND status = 'active' FOR UPDATE"
    );
    ...
    try {
      ...
      if (canaleLog) {
        const embedTasse = new EmbedBuilder()
          .setColor('#ff0000')
          .setTitle('Ministero delle Finanze - Tesoreria di Prato')
          .setDescription(
            'AVVISO FISCALE AUTOMATICO\n\n' +
            'I sistemi centrali di Banca Toscana hanno eseguito il prelievo automatizzato della Tassazione Settimanale.\n\n' +
            'Aliquota applicata: 12% sul saldo disponibile\n' +
            'Totale gettito: Euro ' + formatEuro(gettitoTotale) + ',00\n' +
            'Destinatario: Tesoreria dello Stato (Prato)\n\n' +
            'I prelievi vengono eseguiti ogni 7 giorni.'
          )
          .setTimestamp()
    ...
    console.log('Tassazione completata. Riscossi Euro ' + formatEuro(gettitoTotale));
  } catch (err) {
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isButton() && interaction.customId === 'apri_conto_toscana') {
    ...
    if (ruoloApprovato) {
      return interaction.reply({ content: 'Hai gia un conto corrente in Banca Toscana!', ephemeral: true });
    }

    const ruolo = interaction.member.roles.cache.get(RUOLO_RICHIESTA_ID);
    if (!ruolo) {
      return interaction.reply({ content: 'Devi avere il ruolo <@&' + RUOLO_RICHIESTA_ID + '> per aprire un conto corrente.', ephemeral: true });
    }
    ...
    modal.addComponents(
      new ActionRowBuilder().addComponents(nomeCognomeInput),
      new ActionRowBuilder().addComponents(nomeRobloxInput)
    );

    await interaction.showModal(modal);
  }

  // Modal apertura conto
    ...
    try {
      ...
      if (canaleLog) {
        const embedRichiesta = new EmbedBuilder()
          ...
          .setDescription('Una nuova richiesta di apertura conto corrente e in sospeso.')
          .addFields(
            { name: 'Discord', value: interaction.user.toString() + ' (' + interaction.user.tag + ')', inline: true },
        ...
        const rowBottoni = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId('approva_conto_' + userId).setLabel('Approva').setStyle(ButtonStyle.Success),
          new ButtonBuilder().setCustomId('rifiuta_conto_' + userId).setLabel('Rifiuta').setStyle(ButtonStyle.Danger)
        );
      ...
      return interaction.editReply({ content: 'La tua richiesta di apertura conto e stata inviata! Attendi l\'approvazione dello staff.' });

    } catch (err) {
      console.error('Errore durante il form apertura conto:', err);
      return interaction.editReply({ content: 'Errore durante l\'invio della richiesta. Riprova piu tardi.' });
    }
  }
  ...
  if (interaction.isButton() && interaction.customId.startsWith('approva_conto_')) {
    ...
    try {
      ...
      if (!datiRichiesta) {
        return interaction.reply({ content: 'Richiesta non trovata o gia processata.', ephemeral: true });
      }
      ...
      try {
        ...
        const saldoTesoreria = parseInt(tesoreriaRes.rows[0] ? tesoreriaRes.rows[0].saldo : '0', 10);

        if (saldoTesoreria < 5000) {
          await clientDb.query('ROLLBACK');
          return interaction.reply({ content: 'Errore: I fondi della Tesoreria di Prato sono esauriti.', ephemeral: true });
        }
        ...
        // Assegna il ruolo — con gestione errore se il ruolo non esiste
        try {
          ...
          // Il ruolo non esiste o il bot non ha i permessi — logghiamo ma non blocchiamo
          console.error('Errore assegnazione ruolo (verifica RUOLO_APPROVATO_ID nelle env):', roleErr.message);
        }
        ...
        try {
          ...
          if (user) {
            const embedBenvenuto = new EmbedBuilder()
              .setColor('#006400')
              .setTitle('Banca Toscana - Conto Approvato')
              .setDescription('Benvenuto in Banca Toscana, ' + datiRichiesta.username + '!\nIl tuo conto corrente e stato APPROVATO ed e ora operativo.')
              .addFields(
                ...
                { name: 'Accredito di Benvenuto', value: 'Euro 5.000,00', inline: false }
              )
              .setTimestamp()
          ...
          console.error('Impossibile inviare DM all\'utente:', dmErr.message);
        }
        ...
        return interaction.reply({ content: 'Conto corrente approvato per <@' + userId + '>!', ephemeral: true });
      ...
      return interaction.reply({ content: 'Errore durante l\'approvazione del conto.', ephemeral: true });
    }
  }
  ...
  if (interaction.isButton() && interaction.customId.startsWith('rifiuta_conto_')) {
    ...
    try {
      ...
      try {
        ...
        if (user) {
          const embedRifiuto = new EmbedBuilder()
            .setColor('#ff0000')
            .setTitle('Banca Toscana - Conto Rifiutato')
            .setDescription('Spiacenti, la tua richiesta di apertura conto e stata RIFIUTATA.')
            .addFields(
              { name: 'Nome e Cognome (RP)', value: datiRichiesta.nomeCognome, inline: true },
              { name: 'Nome Roblox', value: datiRichiesta.nomeRoblox, inline: true }
            )
        ...
        console.error('Impossibile inviare DM:', dmErr.message);
      }
      ...
      richiestePendenti.delete(userId);
      return interaction.reply({ content: 'Richiesta rifiutata per <@' + userId + '>. Notifica inviata via DM.', ephemeral: true });

    } catch (err) {
      console.error('Errore durante rifiuto conto:', err);
      return interaction.reply({ content: 'Errore durante il rifiuto del conto.', ephemeral: true });
    }
  }

  if (!interaction.isChatInputCommand()) return;

  // Comando: invia_pannello
  if (interaction.commandName === 'invia_pannello') {
    ...
    const embedIniziale = new EmbedBuilder()
      .setColor('#006400')
      .setTitle('Servizi Bancari | Apri il tuo conto corrente')
      .setDescription(
        ...
        '- Monitorare i tuoi risparmi in modo sicuro e immediato\n' +
        '- Inviare bonifici bancari per pagare tasse, stipendi o transazioni commerciali\n' +
        '- Accedere in tempo reale alle funzioni digitali di home banking\n\n' +
        'Il sistema elaborera la tua richiesta ed erogherà i fondi in tempo reale.'
      )
      .setFooter({ text: 'Banca Toscana S.p.A. - Sicurezza e stabilita per il cittadino' });
  ...
  if (interaction.commandName === 'saldo_toscana') {
    ...
    try {
      ...
      if (res.rows.length === 0 || res.rows[0].status !== 'active') {
        return interaction.editReply({ content: 'Non hai ancora un conto aperto o è in sospeso.' });
      }
      const saldoAttuale = parseInt(res.rows[0].saldo, 10) || 0;
      const embedSaldo = new EmbedBuilder()
        .setColor('#006400')
        .setTitle('Banca Toscana - Servizi Online')
        .setDescription('Buon pomeriggio, ' + interaction.user.toString() + '!\n\nSaldo Disponibile: Euro ' + formatEuro(saldoAttuale) + ',00')
        .setTimestamp()
        .setFooter({ text: 'Banca Toscana S.p.A.' });
  ...
  if (interaction.commandName === 'bonifico_toscana') {
    ...
    try {
      ...
      if (targetRes.rows.length === 0 || targetRes.rows[0].status !== 'active') {
        await clientDb.query('ROLLBACK');
        return interaction.editReply({ content: 'Il beneficiario non ha un conto attivo in Banca Toscana.' });
      }
      ...
      const embedBonifico = new EmbedBuilder()
        ...
        .setDescription('Trasferimento fondi completato con successo.')
        .addFields(
          { name: 'Beneficiario', value: target.toString() + ' (' + target.tag + ')', inline: true },
          { name: 'Importo', value: 'Euro ' + formatEuro(importo) + ',00', inline: true },
          { name: 'Causale', value: causale, inline: false }
        )
        .setTimestamp()
        .setFooter({ text: 'Operazione tracciata da Banca Toscana' });

      return interaction.editReply({ embeds: [embedBonifico] });
    } catch (err) {
      try { await clientDb.query('ROLLBACK'); } catch (_) {}
      console.error('Errore bonifico:', err);
      return interaction.editReply({ content: 'Errore durante il trasferimento dei fondi.' });
    } finally {
      clientDb.release();
    }
  }
